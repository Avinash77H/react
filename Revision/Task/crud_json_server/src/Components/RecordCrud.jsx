...
const RecordCrud = () => {
  ...
  // get all exists json-server data for initial render on page
  useEffect(() => {
    fetchData();
  }, []);

  // get input value onChange event
  ...
  // send input data to json-server when submit the form
  const handlleSubmit = async(e) => {
    e.preventDefault();

    if(editId){
      // if exitId exists, update the user
      await axios.put(`http://localhost:3000/user/${editId}`,formData);
    }else{
      // if editId does not exits, add a new user
  ...
  return (
    <div className="container mx-auto bg-white rounded-lg shadow-md p-6">
      <h1 className="text-3xl font-bold mb-4 text-center">CRUD</h1>
